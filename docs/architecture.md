...
| [`_auth/cookies.py`](../src/notebooklm/_auth/cookies.py) | Compatibility cookie maps/converters, `_update_cookie_input`, and thin recovery-composition adapters over the dependency-bottom paired loader. Each successfully converted live cookie supplies identity/value/runtime fields while SameSite comes from that same sanitized raw row, preserving first-successful duplicate behavior in the typed baseline. |
| [`_auth/cookie_pair.py`](../src/notebooklm/_auth/cookie_pair.py) | Dependency-bottom one-sample storage-state loader and paired live `httpx`/typed `CookieJar` projection. It owns storage validation, sanitization, required-cookie checks, duplicate selection, provenance-preserving conversion, and the historical validation/error values; it imports no compatibility cookie, recovery, or profile-store module. |
| [`_auth/cookie_contract.py`](../src/notebooklm/_auth/cookie_contract.py) | Dependency-bottom immutable RotateCookies endpoint contract shared by cookie routing and the mint/recovery wire adapters. |
...
| `_auth/cookies.py` | Compatibility cookie loaders/converters and recovery adapters over the pure paired loader + `_update_cookie_input` |
| `_auth/cookie_pair.py` | Dependency-bottom one-sample storage loader and live/typed cookie-pair projection |
| `_auth/cookie_contract.py` | Dependency-bottom RotateCookies endpoint contract |
...
│   ├── cookies.py               # Compatibility cookie loaders/converters + recovery adapters + _update_cookie_input
│   ├── cookie_pair.py           # Dependency-bottom one-sample loader and paired live/typed projection
│   ├── cookie_contract.py       # Dependency-bottom RotateCookies endpoint contract
