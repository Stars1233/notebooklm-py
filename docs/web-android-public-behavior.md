...
- [#2384](https://github.com/teng-lin/notebooklm-py/issues/2384) for the
  `chat.get_history` strictness fix (out of scope here)
...
| `chat.get_history` on turn-fetch failure | Web maps `get_conversation_turns` `ChatError` / `NetworkError` to `[]` (empty history is then indistinguishable from a failed turn fetch). Android lets the same failure propagate. No-conversation (`get_conversation_id` → `None`) is a real empty and stays `[]` on both backends. | Bug / missing contract — do **not** silently "fix" here; tracked as [#2384](https://github.com/teng-lin/notebooklm-py/issues/2384) | Web `[]`: `tests/unit/test_chat_characterization.py` (`test_get_history_returns_empty_on_chat_error`, `test_get_history_returns_empty_on_network_error`). Android raise: `tests/unit/android/test_chat.py` (`test_get_history_propagates_turn_fetch_failures`) |
